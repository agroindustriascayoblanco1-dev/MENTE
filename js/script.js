...
function setMood(mood){
  ...
  const d=moods[mood];
  $("#selectedMood").textContent=d.emoji;
  $("#supportTitle").textContent=d.title;
  $("#supportText").textContent=d.text;
  $("#wisdomType").textContent=d.type;
  $("#wisdomText").textContent=d.wisdom;
  $("#supportEyebrow").textContent=mood==="very-sad"?"VAMOS A CUIDARTE":"PARA ESTE MOMENTO";
  showScreen("supportScreen");
}

function renderNeeds(){
  const list=$("#needsGrid");
  list.innerHTML="";
  (moodNeeds[currentMood]||moodNeeds.okay).forEach(k=>{
    const n=needs[k];
    const b=document.createElement("button");
    b.className="need-card";
    b.dataset.need=k;
...
function chooseNeed(key){
  ...
  const alt=$("#alternateTools");
  alt.innerHTML="";
  (moodNeeds[currentMood]||[]).filter(x=>x!==key).slice(0,2).forEach(k=>{
    const a=needs[k],b=document.createElement("button");
    b.textContent=`También puedo: ${a.icon} ${a.title}`;
    b.dataset.need=k;
    alt.appendChild(b);
  });
...
function runAction(action){
  if(action==="breathing"){
    setupActivity("🌬️","PAUSA","Respirar conmigo","Inhala cuando la esfera crezca. Exhala cuando vuelva a hacerse pequeña.");
  }else if(action==="game"){
    setupGame();
  }else if(action==="thoughts"){
    showScreen("journalScreen");
    $("#journalText").placeholder="Escribe qué está ocupando espacio en tu cabeza...";
  }else if(action==="connect"){
    showScreen("connectScreen");
  }else if(action==="help"){
    showScreen("helpScreen");
  }else{
    setupActivity("🌱","PEQUEÑO PASO","Haz una cosa posible","Elige una acción pequeña y amable que puedas hacer durante los próximos cinco minutos.");
  }
}
...
function setupActivity(icon,label,title,desc){
  ...
  $("#activityDescription").textContent=desc;
  $("#activityMessage").textContent="";
  $("#breathingCount").textContent="";
  $("#breathingOrb").classList.remove("breathe");
  $("#breathingWord").textContent="Respira";
  $("#activityStart").textContent="Comenzar";
...
function startBreathing(){
  ...
  if(breathingInterval){
    ...
    word.textContent="Pausa";
    msg.textContent="Muy bien. Quédate con una respiración tranquila.";
    $("#activityStart").textContent="Comenzar";
    return;
  }
  ...
  breathCycle=0;
  let phase=0;
  $("#activityStart").textContent="Pausar";
  msg.textContent="Inhala lentamente…";
  word.textContent="Inhala";
  breathingInterval=setInterval(()=>{
    phase++;
    if(phase%2===0){
      breathCycle++;
      count.textContent=`Respiración ${Math.min(breathCycle,5)} de 5`;
      word.textContent="Inhala";
      msg.textContent="Inhala lentamente…";
    }else{
      word.textContent="Exhala";
      msg.textContent="Suelta el aire despacio…";
    }
    if(breathCycle>=5){
      clearInterval(breathingInterval);
      breathingInterval=null;
      ...
      $("#activityStart").textContent="Comenzar";
      msg.textContent="Terminaste. Quédate unos segundos aquí.";
      setTimeout(()=>showScreen("afterScreen"),900);
    }
  },4000);
}
...
function spawnFirefly(){
  ...
  const b=document.createElement("button");
  b.className="firefly";
  b.setAttribute("aria-label","Luciérnaga");
  b.style.left=(7+Math.random()*85)+"%";
  b.style.top=(7+Math.random()*78)+"%";
  b.onclick=()=>{
    fireflyScore++;
    updateScore();
    spawnFirefly();
...
function updateScore(){
  $("#gameScore").textContent=`${fireflyScore} ${fireflyScore===1?"luz encontrada":"luces encontradas"}`;
}
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  // Un solo listener para la navegación y botones con data-go.
  document.addEventListener("click",e=>{
    ...
    const mood=e.target.closest(".mood-card");
    if(mood){
      e.preventDefault();
    ...
    const need=e.target.closest(".need-card,[data-need]");
    if(need){
      e.preventDefault();
      chooseNeed(need.dataset.need);
      return;
    }
    ...
    if(category){
      e.preventDefault();
      runAction(category.dataset.action);
      return;
    }

    if(e.target.closest("#helpBtn")){e.preventDefault();renderNeeds();return;}
    ...
    if(e.target.closest("#activityDone")){e.preventDefault();stopActivity();return;}
    ...
    if(e.target.closest("#soundBtn")){e.preventDefault();showToast("El ambiente sonoro será el siguiente módulo.");return;}
    if(e.target.closest("#saveJournal")){
      e.preventDefault();
      ...
      $("#savedNote").textContent="✓ Guardado en este dispositivo.";
    ...
    if(after){
      e.preventDefault();
      const response={
        better:"Me alegra que haya cambiado un poquito. Un pequeño cambio también cuenta. 💛",
        same:"Está bien. No todo cambia en un solo momento. Podemos probar otra cosa.",
        worse:"Gracias por decírmelo. Si esto se siente demasiado difícil, busca compañía y considera pedir ayuda ahora."
      };
      $("#afterResponse").textContent=response[after.dataset.after];
      $("#afterEmoji").textContent=after.dataset.after==="better"?"🌤️":after.dataset.after==="same"?"🌱":"🫶";
    }
  });

  const saved=localStorage.getItem("mente_journal");
  if(saved){
    try{$("#journalText").value=JSON.parse(saved).text||"";}catch(_){}
  }
});
